...
sampleN = 10000;                            % Number of Monte Carlo samples
...
currentDay = 0;
dayIdx = find(days <= currentDay + 1);
findProb = zeros(numArms, max(days));
...
for I = 1 : numVisitors
    ...
    if  I == dayIdx(end - 1)
        inputPrior(1,:,:) = ones(size(numSucc(1,:,:))) + numSucc(countI,:,:) ;
        inputPrior(2,:,:) = ones(size(numSucc(1,:,:))) + ...
            numTrials(countI,:,:) - numSucc(countI,:,:) ;
        findProb(:,currentDay + 1) = monteCarlo(sampleN, inputPrior, numArms, numTypes, allUsers);
