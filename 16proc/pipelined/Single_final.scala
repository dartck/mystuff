...
class Dpath extends Module {
  ...
  val alu_in1 = Bits()  // See forwarding unit
  val alu_in2 = Bits()  // See forwarding unit
  val alu_out = MuxCase(UInt(0), Array(
              ...
              (idexsig_aluop === C.ALU_SLT) -> (Mux(alu_in1 < alu_in2, UInt(1), UInt(0))).toUInt,
  ...
  val jumpaddr = Mux(idexsig_jrmux, regfile(idex_rsi), idex_jumpad)
  ...
  exmem_writedata := regfile(idex_rti)
...
// Forwarding unit
// Checks for common registers between inst. to forward

// Input 1
  when (idex_rsi === exmem_dstreg && exmem_dstreg != UInt(0) && exmemsig_regwrite){
     alu_in1 := exmem_aluout
  } .elsewhen (idex_rsi === memwb_dstreg && memwb_dstreg != UInt(0) && memwbsig_regwrite){
     alu_in1 := Mux(memwbsig_memtoreg, memwb_dmemout, memwb_aluout)
  } .otherwise {
     alu_in1 := regfile(idex_rsi)
  } 

// Input 2
  when (idex_rti === exmem_dstreg && exmem_dstreg != UInt(0) && exmemsig_regwrite && idexsig_alusrc){
     alu_in2 := exmem_aluout
  } .elsewhen (idex_rti === memwb_dstreg && memwb_dstreg != UInt(0) && memwbsig_regwrite && idexsig_alusrc){
     alu_in2 := Mux(memwbsig_memtoreg, memwb_dmemout, memwb_aluout)
  } .otherwise {
     alu_in2 := Mux(idexsig_alusrc, regfile(idex_rti), idex_sextimm)
  } 


// Hazard Detection Unit
// Stalls on cases where branching or jumping is required or if there is a load-use scenario.

  when (io.branch || idexsig_branch || io.jump || idexsig_jump){
     stall := Bool(true)
     ifid_inst := nop
  } .elsewhen ((instruction (20,16) === rti || instruction (25,21) === rti) && io.mem_toreg){
     stall := Bool(true)
     ifid_inst := nop
  } .otherwise {
     stall := Bool(false)
