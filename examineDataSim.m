...
classDataSim = zeros(size(convData)) ;
...
awakeHrs = timeVal(timeIdx(1:8)) ; % 14 hour awake time
...
if wrapCheck == 23
    sleepHrs = timeVal(timeIdx(9:24)) ;
    minHr = min(sleepHrs) ; maxHr = max(sleepHrs) ;
    classDataSim(find(tmpData < minHr | tmpData > maxHr),1) = 1 ;
else
...
for I = 1 : length(bigDataVal)
    classDataSim( find(convData(:,4) == bigDataVal(I)), 4 ) = 1 ;
end
...
close(gcf) ;
[~,dataIdx] = sort(dataCounts,1,'descend') ;
bigDataVal = dataVal(dataIdx(1:4)) ; % by eye
% Classify CData as Big or Small
for I = 1 : length(bigDataVal)
    classDataSim( find(convData(:,5) == bigDataVal(I)), 5 ) = 1 ;
end
...
bigDataVal = dataVal(dataIdx(1:2)) ; % by eye
% Classify CData as Big or Small
classDataSim( find(convData(:,6) == bigDataVal(1)), 6 ) = 2 ;
classDataSim( find(convData(:,6) == bigDataVal(2)), 6 ) = 1 ;

classDataSim(:,6) = -1*(classDataSim(:,6) - 2);
classDataSim(:,7) = [];
classDataSim(:,3) = [];
